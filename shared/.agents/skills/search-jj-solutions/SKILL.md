---
name: search-jj-solutions
description: Search for the best solution by fanning divergent jj revisions against a frozen judge and harvesting the winning path. Use when the approach is unknown and must be discovered through measured parallel attempts rather than planned stages.
...
Use `plan-jj-work` when the steps are known. Use this when the *winning approach* is unknown
...
1. **Pin the base.** One jj revision the whole search descends from — a `plan-jj-work` stage,
...
2. **Fork-fan-out.** For each frontier node, spawn J children — one divergent idea each,
   each in its own Pando workspace created with `pando`, forked from that node, each carrying the journal and
