...
A skill whose `agents/openai.yaml` permits implicit invocation, so the agent can fire it autonomously and the human can still type its name. It pays permanent **context load** for that discoverability. A model-invoked skill whose content is all **reference** can also be shared reference that other skills reach. Pick model-invocation only when the agent must reach the skill on its own; otherwise set `policy.allow_implicit_invocation: false`.

_Avoid_: ability, tool, capability

### User-Invoked

A skill with `policy.allow_implicit_invocation: false` in `agents/openai.yaml`, reachable only when the human types its name. It trades agent-discoverability for zero **context load**. Its required description is human-facing rather than an autonomous trigger.

_Avoid_: procedure, workflow, command

### Description

The skill's required summary. For a **model-invoked** skill it is a machine-readable trigger and top-level **context pointer**. For a **user-invoked** skill it is a concise human-facing summary; `agents/openai.yaml`, not the description's presence, controls invocation.
...
The cost a **model-invoked** skill imposes on the agent's context window — its **description**, always loaded, spending both tokens and attention. What **user-invoked** skills escape through invocation policy, and the brake on splitting into more model-invoked skills.
...
A **user-invoked** skill whose job is to point at your other user-invoked skills — naming each and when to reach for it — so the human has one skill to remember instead of many. It can only hint, never fire them because their policy requires explicit invocation. The cure for **cognitive load** when user-invoked skills multiply.
...
**Reference** that lives outside the skill system — a plain file, no **description**, no **steps**, not invocable — that any skill can point at. It is the shared home for material that needn't fire on its own, including material shared by user-invoked skills that cannot invoke one another.
