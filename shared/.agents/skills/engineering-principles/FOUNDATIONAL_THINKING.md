...
Define the core types early and trace every access pattern before you settle them, so the structure matches the paths that actually dominate. How to model those types well, making illegal states unrepresentable and parsing at the boundary, is owned by `../apply-coding-standards/DOMAIN_MODELING.md` and `../apply-coding-standards/BOUNDARIES_AND_PARSING.md`. This principle is about doing that thinking first, before the logic, not about the modeling rules themselves.
