...
Do the first unit by hand to learn the recipe, then build the lever and rerun it on that same unit, diffing against your hand-done version to prove it matches. Make the lever safe to rerun, because a reviewer will rerun it. For edits, that is a codemod or a script; use structural-search tooling such as `ast-grep` when the change is syntax-shaped. For analysis, it is a query over dumped data. For verification, it is a rerunnable check, which is `PROVE_IT_WORKS.md` applied to the proof itself.

A lever pass is complete when the tool has run on at least one known unit, its output has been compared to the expected hand result, it is safe to rerun, and the final diff or report comes from the lever rather than from unverified manual repetition.
