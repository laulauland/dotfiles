...
description: Execution principles for scoping, sequencing, debugging, and verifying engineering work. Use when planning or sizing a change, driving an autonomous run, deciding what done means, or auditing a codebase for architecture that should be deepened.
---

# Engineering Principles

These are execution principles: how to drive a piece of work from request to verified result. They are orthogonal to code-design taste. When a decision is about naming, types, modules, parsing, errors, or tests, that is the `apply-coding-standards` skill, and these files defer to it rather than restating it.
...
## How this relates to apply-coding-standards

`apply-coding-standards` answers "is this code well-designed?" These principles answer "am I working on it the right way?" Where they touch the same ground, the principle points at the standard:

- Verification of a finished task lives here (`PROVE_IT_WORKS.md`); what a good test looks like lives in `../apply-coding-standards/TESTING_AND_VERIFICATION.md`.
- Diff size and deletion bias live here; module and interface design lives in `../apply-coding-standards/DESIGNING_MODULES.md`.
- Target-state-not-migration sequencing lives here; the no-backwards-compat design rule lives in `apply-coding-standards`'s non-negotiables.
...
| Auditing a codebase for shallow modules and deepening opportunities | [ARCHITECTURE_AUDIT.md](ARCHITECTURE_AUDIT.md) and [ARCHITECTURE_REPORT.md](ARCHITECTURE_REPORT.md) |
